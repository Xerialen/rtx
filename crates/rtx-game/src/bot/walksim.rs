...
mod tests {
    ...
    // ------------------------------------------------------------------------------------------
    // TEMPORARY diagnostic probe for the dm3 goto-194 stall investigation. DELETE BEFORE COMMIT.
    // ------------------------------------------------------------------------------------------
    #[test]
    fn tmp_dm3_goto_probe() {
        use rtx_nav::navmesh::{LinkCosts, TeleportInfo, RJ_UNFIT_PENALTY};
        ...
        let Ok(bytes) = std::fs::read(std::path::Path::new(&dir).join("dm3.bsp")) else {
            eprintln!("no dm3.bsp; skipping");
            return;
        };
        let bsp = crate::bsp::Bsp::parse(&bytes).expect("parse dm3");

        // Minimal teleport parse (mirrors race::tests::teleports_of): trigger box from submodel
        // bounds, dest = target origin +27z.
        let lump = |idx: usize| -> &[u8] {
            let at = 4 + idx * 8;
            let ofs = i32::from_le_bytes(bytes[at..at + 4].try_into().unwrap()) as usize;
            let len = i32::from_le_bytes(bytes[at + 4..at + 8].try_into().unwrap()) as usize;
            &bytes[ofs..ofs + len]
        };
        let ents = String::from_utf8_lossy(lump(0)).to_string();
        let models = lump(14).to_vec();
        let mut blocks: Vec<Vec<(String, String)>> = Vec::new();
        {
            let mut cur: Option<Vec<(String, String)>> = None;
            let mut strings: Vec<String> = Vec::new();
            let mut chars = ents.chars().peekable();
        ...
        let field = |b: &[(String, String)], key: &str| -> Option<String> {
            b.iter().find(|(k, _)| k == key).map(|(_, v)| v.clone())
        };
        let vec3_of = |s: &str| -> Vec3 {
        ...
        let submodel = |n: usize| -> (Vec3, Vec3) {
            let rec = &models[n * 64..n * 64 + 24];
            let f = |i: usize| f32::from_le_bytes(rec[i * 4..i * 4 + 4].try_into().unwrap());
            (Vec3::new(f(0), f(1), f(2)), Vec3::new(f(3), f(4), f(5)))
        };
        let teleports: Vec<TeleportInfo> = blocks
            .iter()
            .filter(|b| field(b, "classname").as_deref() == Some("trigger_teleport"))
            .filter_map(|b| {
                let n: usize = field(b, "model")?.strip_prefix('*')?.parse().ok()?;
                let (tmin, tmax) = submodel(n);
                let target = field(b, "target")?;
                let dest = blocks
                    .iter()
                    .find(|d| field(d, "targetname").as_deref() == Some(target.as_str()))?;
                let mut origin = field(dest, "origin").map(|s| vec3_of(&s))?;
                if field(dest, "classname").as_deref() == Some("info_teleport_destination") {
                    origin.z += 27.0;
                }
                Some(TeleportInfo {
                    tmin,
            ...
            .collect();
        eprintln!("teleports: {}", teleports.len());

        // Live harness build: rjtest.cfg has grapple 0, doublejump 0, bhop 1, rocketjump 1;
        // curljump defaults on; sv_* at stock. Plats NOT spliced (all three sit far east of the area
        // under test).
        let graph = rtx_nav::navmesh::build_navmesh(
            &bsp,
            Vec::new(),
            ...
            Some(rtx_nav::navmesh::RocketJumpParams {
                gravity: 800.0,
                rj_extra: 1.0,
            }),
        );

        let show = |c: rtx_nav::navmesh::CellId| {
            ...
            format!(
                ...
                o.y,
                o.z,
                if graph.is_ledge(c) { "L" } else { "" }
            )
        };
        // Identity check against the live ids.
        for (pos, want) in [
            (Vec3::new(-512.0, 864.0, 120.0), 527u32),
            (Vec3::new(-800.0, 96.0, 184.0), 194),
            (Vec3::new(-192.0, 544.0, 8.0), 839),
            (Vec3::new(-192.0, 768.0, 120.0), 846),
        ] {
            let got = graph.nearest(pos).unwrap();
            eprintln!("nearest({pos:?}) = {} (live said {want})", show(got));
        }
        ...
        let fit = LinkCosts::default();

        let print_route = |label: &str, r: &Option<Vec<u32>>| match r {
            None => eprintln!("{label}: NO PATH"),
            Some(r) => {
                let mut s = String::new();
                let mut total = 0.0f32;
                for &li in r {
                    total += graph.link_cost(li);
                    s.push_str(&format!(
                        " [{li} {:?} {}->{} c={:.2}]",
                        graph.link_kind(li),
                        show(graph.link_source(li)),
                        show(graph.link_target(li)),
                        graph.link_cost(li)
                    ));
                }
                eprintln!("{label}: {} legs, static cost {:.2}\n   {}", r.len(), total, s);
            }
        };

        eprintln!("reachable 527->194: {}", graph.reachable(527, 194));
        eprintln!("reachable 839->194: {}", graph.reachable(839, 194));
        eprintln!("reachable 846->194: {}", graph.reachable(846, 194));

        print_route("unfit 527->194", &graph.find_path(527, 194, &unfit));
        print_route("unfit 839->194", &graph.find_path(839, 194, &unfit));
        print_route("unfit 846->194", &graph.find_path(846, 194, &unfit));
        print_route("fit   839->194", &graph.find_path(839, 194, &fit));

        // All speed-jump links out of 846 and everything out of 839, offline ids.
        for c in [846u32, 839] {
        ...
        // Iterated LOD repath walk under perfect execution: from -> corridor interim -> route ->
        // teleport to route end -> repeat. Detects planning-level cycles.
        let walk = |label: &str, start: u32, costs: &LinkCosts| {
            let mut cur = start;
            eprintln!("LOD walk {label} from {}:", show(cur));
            for i in 0..40 {
                if cur == 194 {
                    eprintln!("  [{i}] arrived");
                    return;
                }
                let corr = graph.corridor(cur, 194, costs, 3.0);
                let (target, window) = match &corr {
                    Some(c) => (c.interim, Some(c.allowed.as_slice())),
                    None => (194, None),
                };
                let route = match window {
                    Some(w) => graph.find_path_within(cur, target, costs, w),
                    None => graph.find_path(cur, target, costs),
                }
                .unwrap_or_default();
                let kinds: Vec<String> = route
                    .iter()
                    .map(|&li| format!("{:?}>{}", graph.link_kind(li), graph.link_target(li)))
                    .collect();
                let end = route.last().map(|&li| graph.link_target(li)).unwrap_or(cur);
                eprintln!(
                    "  [{i}] at {} interim {} route {} legs -> end {} | {}",
                    show(cur),
                    corr.as_ref().map(|c| show(c.interim)).unwrap_or("-".into()),
                    route.len(),
                    show(end),
                    kinds.join(" ")
                );
                if route.is_empty() {
                    eprintln!("  [{i}] EMPTY ROUTE — parked (interim unreachable inside window?)");
                    return;
                }
                cur = end;
            }
            eprintln!("  ...no arrival within 40 repaths");
        };
        walk("unfit", 527, &unfit);
        walk("unfit", 839, &unfit);
        walk("fit", 839, &fit);
    }
}
