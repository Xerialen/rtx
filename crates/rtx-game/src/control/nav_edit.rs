...
// NOTE(merge-trial): the cost-surcharge unlink that lived here lost to the tombstone
// implementation in `super::unlink_json` — a surcharged link still gets routed under enough
// pressure (measured on the dm3 focus route), while the tombstone is skipped by every router.
