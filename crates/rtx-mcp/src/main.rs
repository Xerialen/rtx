...
fn write_config(repo: &std::path::Path, map: &str, port: u16, skill: f32) -> Result<(), String> {
    ...
    //
    // `rtx_weapons` belongs to that same family and was missing, with a worse symptom than a gated
    // link: the enabled-weapon mask is read when each `weapon_*` entity spawns, so on a fresh boot
    // the mask was empty and **every weapon pickup on the map was freed**. The cvar then read back
    // as the full list, so nothing looked wrong — the map simply had no weapons on it, and eight
    // bots ran a whole match with shotguns, which is not a bot bug but reads exactly like one.
    //
    // `maxclients` is 16 rather than the 8 a 4-on-4 needs, because the harness itself occupies a
    // client slot. At 8 the eighth bot of a 4-on-4 was seated past the player range, and every loop
    // written `1..=maxclients` — respawn included — skipped it: a permanent corpse on one team,
    // frozen at its first death with its frags stuck, so the match was silently 3-on-4. It shows up
    // in the roster as an extra name, which is the only outward sign anything is wrong.
    let cfg = format!(
        "// generated by rtx-mcp — the rtx bot control and tuning harness\n\
         ...
         set rtx_weapons \"axe sg ssg ng sng gl rl lg\"\n\
         set rtx_bot_bhop 1\n\
...
impl RtxMcp {
    ...
    #[tool(
        description = "List the maps the running server can load — loose `.bsp` under its gamedirs         plus `maps/*.bsp` inside their pak files, so stock maps like dm3 that live in a pak show up         too. The server answers from its own filesystem, so the list is what it will actually         accept. Optional `filter` keeps only names containing that substring (case-insensitive).         Needs a connected server."
    )]
    async fn list_maps(&self, Parameters(a): Parameters<MapsArgs>) -> Result<CallToolResult, McpError> {
        let maps = self.req(Cmd::Maps, SHORT).await;
        let Some(needle) = a.filter.filter(|f| !f.is_empty()) else {
            return finish(maps);
        };
        let needle = needle.to_ascii_lowercase();
        finish(maps.map(|v| {
            match v {
                Value::Array(names) => Value::Array(
                    names
                        .into_iter()
                        .filter(|n| n.as_str().is_some_and(|n| n.to_ascii_lowercase().contains(&needle)))
                        .collect(),
                ),
                other => other,
            }
