...
mod tele_probe {
    use super::*;
    use rtx_nav::navmesh::LinkCosts;

    /// TEMPORARY: lay the 375 -> 684 route on aerowalk against the teleport trigger volumes.
    ///   RTX_TEST_BASEDIR="$PWD/playground" cargo test -p rtx-waypoint-check --release -- --nocapture tele
    #[test]
    fn tele_route() {
        let Ok(base) = std::env::var("RTX_TEST_BASEDIR") else {
            return;
        };
        let bytes = crate::pak::resolve_bsp(std::path::Path::new(&base), "aerowalk").expect("aerowalk");
        let bsp = Bsp::parse(&bytes).expect("parse");
        let teles = crate::ent::teleports(&bsp);
        let g = build(&bsp);
        println!("mesh: {} cells, {} links", g.cells.len(), g.links.len());

        println!("\n== teleport triggers ==");
        for (i, t) in teles.iter().enumerate() {
            println!(
                "  #{i} box [{:.0} {:.0} {:.0}]..[{:.0} {:.0} {:.0}]  dest [{:.0} {:.0} {:.0}]",
                t.tmin.x, t.tmin.y, t.tmin.z, t.tmax.x, t.tmax.y, t.tmax.z, t.dest.x, t.dest.y, t.dest.z
            );
        }

        let (a, b) = (375u32, 684u32);
        println!("\n== endpoints ==");
        for c in [a, b] {
            let o = g.cell_origin(c);
            println!("  cell {c}: [{:.0} {:.0} {:.0}]", o.x, o.y, o.z);
        }

        let route = g.find_path(a, b, &LinkCosts::default()).expect("a route");
        println!("\n== route: {} legs ==", route.len());
        // The player hull is 32 wide; a path that comes within half of that of the brush touches it.
        const HULL: f32 = 16.0;
        let mut prev = g.cell_origin(a);
        for &li in &route {
            let to = g.cell_origin(g.link_target(li));
            let kind = format!("{:?}", g.link_kind(li));
            // Sample the straight leg and report the closest approach to any trigger box.
            let mut worst: Option<(usize, f32)> = None;
            for s in 0..=20 {
                let p = prev + (to - prev) * (s as f32 / 20.0);
                for (i, t) in teles.iter().enumerate() {
                    let d = box_dist(p, t.tmin, t.tmax);
            ...
            let note = match worst {
                Some((i, d)) if d <= 0.0 => format!("  <== PASSES THROUGH trigger #{i}"),
                Some((i, d)) if d < HULL => format!("  <== within {d:.0}u of trigger #{i} (hull is {HULL:.0})"),
                _ => String::new(),
            };
            println!("  {kind:<9} -> [{:.0} {:.0} {:.0}]{note}", to.x, to.y, to.z);
            prev = to;
        }
    }

    /// Distance from `p` to an AABB; 0 or less when inside.
    fn box_dist(p: Vec3, lo: Vec3, hi: Vec3) -> f32 {
